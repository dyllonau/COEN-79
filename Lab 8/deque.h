...
BLOCKPOINTER_ARRAY_SIZE entries
// NOTE: The default value for the init_bp_array_size is
BLOCKPOINTER_ARRAY_SIZE.
...
#include <math.h>
#include <cassert>
#include "deque_iterator.h" // The iterator class for deque (refer to file
deque_iterator.h)
...
the array of block pointers (The minimum acceptable value is 2)
typedef std::size_t size_type;
typedef Item value_type; // The type of items stored in the deque
typedef deque_iterator<Item> iterator; // A typedef to simplify the use of
iterator class
// CONSTRUCTOR and DESTRUCTOR
deque(size_type init_bp_array_size = BLOCKPOINTER_ARRAY_SIZE, size_type
initi_block_size = BLOCK_SIZE);
...
pointers which points to the last data block
...
+bp_array_index)
{
// If an entry of source.block_pointers is NULL, that entry should be
NULL in this object as well
...
{
//If this is the first_bp of source, then set the first_bp of this
deque
// STUDENT WORK...
if (source.block_pointers + bp_array_index == source.first_bp)
//block_pointers + bp_array_index =
first_bp = block_pointers + bp_array_index;
//If this is the back_ptr of source, then set the back_ptr of this
deque
// STUDENT WORK...
...
// Copy the elements, and set "front_ptr" and "back_ptr" if
appropriate
...
front_ptr = block_pointers[bp_array_index] +
block_item_index;
}
else if (source.block_pointers[bp_array_index] +
block_item_index == source.back_ptr) {
...
// Copy the not-NULL elements of the array of block pointers to the new
array,
// starting at the computer offset
std::copy(first_bp, last_bp + 1, new_block_pointers + 10 +
offsett_first_bp);
...
else if ((back_ptr == ((*last_bp) + (block_size - 1))) && (last_bp !=
block_pointers_end))
{
// STUDENT WORK...
++last_bp;
